# 1) скачать из ВК все посты, которые состоят из 5 строк done
# 2) сделать БД с постами и их id done
# 3) раз в день повторять процедуру выкачивания, начиная с последнего done
# 4) постить случайный порошок раз в три часа done
# 5) захостить на сервере

import tweepy
import time  # работает посекундно, для разницы между постами в три часа надо sleep(10800)
...
# надо как-то сделать так, чтобы посты записывались в дб в порядке возрастания айди
def posts_to_db():
    texts = get_texts()
